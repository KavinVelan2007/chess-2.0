    ...
    while run:
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                ...
                x -= (WINDOW_WIDTH // 2 - WINDOW_HEIGHT // 2)
                row,col = y // (BOARD_HEIGHT // 8),x // (BOARD_WIDTH // 8)
                if bitboard & (uint(1) << uint(row * 8 + col)):
                    curr = (row,col)
        ...
        if curr:
            pygame.draw.rect(display,(255,0,0),(WINDOW_WIDTH // 2 - WINDOW_HEIGHT // 2 + curr[1] * (BOARD_WIDTH // 8),curr[0] * (BOARD_HEIGHT // 8),BOARD_WIDTH // 8,BOARD_HEIGHT // 8),3)

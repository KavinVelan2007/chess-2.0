...
class Preferences(ctk.CTkToplevel):
    ...
    def show(self):
        ...
        self.PieceDropdown = ctk.CTkOptionMenu(
            self,
            variable=self.Piece,
            values=self.ParentObject.PieceOptions,
            width=300,
        )
        self.PieceDropdown.place(x=100, y=10)
        self.BoardDropdown = ctk.CTkOptionMenu(
            self,
            variable=self.Board,
            values=self.ParentObject.BoardOptions,
            width=300,
        )
        self.BoardDropdown.place(x=500, y=10)
        self.ApplyButton = ctk.CTkButton(
            self, command=self.applyPreference, text="Apply"
        )
    ...
    def applyPreference(self):
        ...
        pyobj.execute(
            "update chess set piece = ?, board = ? where username = ?",
            (self.Piece.get(), self.Board.get(), self.ParentObject.UserName),
        )
        sqliteobj.commit()
        self.destroy()

...
class Game(ctk.CTk):
    ...
    def update(self):
        if self.AgainstAI and self.Turn == "B":
            self.PlayBestMove()
        if not self.ValidMoves and self.ChessBoardObj.check_for_check(
            1 if self.Turn == "B" else 0
        ):
            from tkinter import messagebox

            messagebox.showinfo(
                "Game Over", f'{"White" if self.Turn == 'B' else "Black"} Won!'
            )
    ...
    def AddMove(self, move, isCapture):
        ...
        for i in range(12):
            if self.ChessBoardObj.bitboards[i] & (uint(1) << to_index):
                if i in [0, 6]:
                    if isCapture:
                        self.MoveHistory.add_item(
                            chr(97 + from_index % 8) + "x" + square
                        )
                    else:
                    ...
                    self.MoveHistory.add_item(
                        self.PieceIndex[i] + ("x" if isCapture else "") + square
                    )
